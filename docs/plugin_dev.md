...
| `to_exec` | string[] | 要求主程序代为执行的命令列表。每条命令走与用户输入完全相同的解析+执行流程。非空时，主程序执行后会将结果填回 `exec_result` 发回。结果已用嵌套提示符 `@[cmd] #>` 标注命令来源 |
| `exec_result` | string[] \| null | `to_exec` 的执行结果数组（主程序填充后发回）。数组顺序与 `to_exec` 一一对应，每条结果已标注对应的命令。仅出现在主程序发回给插件的 response 中 |
